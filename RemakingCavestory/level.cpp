	...
	size(Vector2(0, 0))
{
	...
	this->playBackgroungMusic(mapName);
}
...
void Level::loadMap(std::string mapName, Graphics &graphics)
{
	...
	//Get the width and the height of the whole map and store it in size
	int width, height;
	mapNode->QueryIntAttribute("width", &width);
	mapNode->QueryIntAttribute("height", &height);
	this->size = Vector2(width, height);

	//Get the width and the heigth of the tiles and store it in titleSize
	...
	if (pTileset != NULL)
	{
		while (pTileset)
		{
			...
			//Get all the animations for the tileset
			...
			if (pTileA != NULL)
			{
				while (pTileA)
				{
					...
					ati.StartedTileId = pTileA->IntAttribute("id") + firstgid;
				...
				while (pData)
				{
					...
					if (pTile != NULL)
					{
						...
						while (pTile)
						{
							...
							//If !gid, no tile should be drawn. Continue loop
							if (!pTile->IntAttribute("gid"))
							{
								tileCounter++;
							...
							{
								if (this->animatedTileInfos.at(i).StartedTileId == gid)
								{
	...
	if (pObjectGroup != NULL)
	{
		while (pObjectGroup)
		{
			...
			// Other objecttsgroups go here with an else
				...
				if (pObject != NULL)
				{
					while (pObject)
					{
						...
						if (pPolyline != NULL)
						{
							...
							//and split them into Vector2s and then store them in our points vectors
			...
			else if (ss.str() == "enemies") 
			{
				...
				if (pObject != NULL) 
				{
					while (pObject) 
					{
						...
						if (ss.str() == "bat") 
						{
							this->enemies.push_back(std::make_shared<Bat>(graphics, Vector2(std::floor(x) * globals::SPRITE_SCALE,
...
void Level::playBackgroungMusic(std::string mapName)
{
	//SDL_Init(SDL_INIT_AUDIO);
	//initAudio();
	if (mapName == "Map_1")
		playMusic("content/music/Gestation.wav", SDL_MIX_MAXVOLUME / 2);
	if (mapName == "Map_2")
		playMusic("content/music/Quiet.wav", SDL_MIX_MAXVOLUME / 2);
}
...
std::vector<Slope> Level::checkSlopeCollision(const Rectangle &other)
{
	...
	{
		if (this->slopes.at(i).collidesWith(other))
		{
			others.push_back(this->slopes.at(i));
		}
	}
	return others;
}
...
std::vector<Door> Level::checkDoorCollision(const Rectangle &other)
{
	...
	for (int i = 0; i < this->doorList.size(); i++)
	{
		...
		{
			others.push_back(this->doorList.at(i));
		}
	}
	return others;
}
...
Vector2 Level::getTilesetPosition(Tileset tls, int gid, int tileWidth, int tileHeight)
{
	...
	int tsxx = gid % (tilesetWidth / tileHeight) - 1;
	tsxx *= tileWidth;
	int tsyy = 0;
